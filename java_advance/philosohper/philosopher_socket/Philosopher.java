...
/*
 * Each philosopher communicates with the server (which manages the forks) over a socket connection. 
 * The server can handle multiple connections at the same time, each possibly from a different thread(philosopher).
 * the Philosopher class implements the Runnable interface, so an instance of it can be communicates with the server over a socket to pick up and put dow forks
 */
